...
```sql
SELECT numero, nome, defesa FROM pokemon ORDER BY total DESC;
```
...
```sql
SELECT nome FROM pokemon WHERE nome = 'Pikachu' AND nome = 'Bulbasaur' AND nome = 'Charmander' ORDER BY total DESC;  
```
...
```sql
SELECT numero, nome, total, taxa_captura FROM pokemon WHERE lendaria = 1 ORDER BY taxa_captura DESC, total DESC LIMIT 5;
```
...
```sql
SELECT numero, nome, tipo1, tipo2, lendario FROM pokemon WHERE tipo1 = 'normal' AND tipo2 IS NULL AND NOT nome IN('Regigigas', 'Arceus');
```
...
```sql
SELECT numero, nome, tipo1, tipo2, cor FROM pokemon WHERE NOT cor = 'Azul' ORDER BY nome ASC;
```
